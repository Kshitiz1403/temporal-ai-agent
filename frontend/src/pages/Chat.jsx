...
export default function Chat() {
    ...
    }, [conversation, lastMessage, scrollToBottom]);

    // Handle resuming existing conversation from URL
    useEffect(() => {
        if (conversationId && conversationId !== 'new') {
            console.log('Attempting to resume conversation:', conversationId);
            
            // Set the session ID and try to fetch conversation
            apiService.currentSessionId = conversationId;
            setSessionStarted(true);
            setDone(false);
            
            // Fetch conversation history immediately
            fetchConversationHistory();
        }
    ...
    const handleSendMessage = async () => {
        ...
        if (!trimmedInput || loading || done) return;
    ...
    const handleStartNewChat = async () => {
        try {
            ...
            if (result.data && result.data.sessionId) {
                navigate(`/chat/${result.data.sessionId}`);
            }
